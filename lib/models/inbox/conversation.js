      ...
      messageModel.createNew(conv, from, content, function (err, cMsg) {
        if (err) {
          ...
          conv.members.forEach(function (user, i) {
            messageReadModel.createNew(user, cMsg, user._id.toString() === from.toString(), function (err, cMsgRead) {
              if (err) {
                cb(err);
              } else {
...
function getMessageRead(user, message, cb) {
  messageReadModel.getByUserMessage(user, message, function (err, fRead) {
    if (err) {
      cb(err);
    } else {
      cb(null, fRead.read);
