...
messageReadSchema.statics.getByConversationsAndUser = function (conversations, user, read, cb) {
  messageReadModel.find({conversation: { $in: conversations }, user: user, read: read}, cb);
};

messageReadSchema.statics.updateReadConversation = function (user, conversation, read, cb) {
  messageReadModel.update({ user: user, conversation: conversation, read: !read }, { read: read }, { multi: true }, cb);
};
