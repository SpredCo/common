...
spredcastSchema.statics.findAvailableCast = function (filters, cb) {
  if (filters.tags === undefined) {
    spredcastModel.find({ $or: [{ date: { $gt: new Date() }, state: 0}, { state: 1 }]}).populate('creator tags').exec(function (err, fCasts) {
      if (err) {
        cb(err);
      } else {
    ...
    spredcastModel.find({ $or: [{ date: { $gt: new Date() }, state: 0}, { state: 1 }], tags: {$in: filters.tags}}).populate('creator tags').exec(function (err, fCasts) {
      if (err) {
        cb(err);
      } else {
        const cast = filterByState(filters, fCasts);
...
function filterByState (filter, fCasts) {
  var rep =[];
  var states = [];
  if (filter.state !== undefined && Array.isArray(filter.state)) {
    filter.state.forEach(function (elem) {
      states.push(parseInt(elem));
    });
  }
  fCasts.forEach(function (fCast) {
    if (filter.state === undefined || filter.state === fCasts.state || (Array.isArray(filter.state) && filter.state.indexOf(fCast.state) !== -1)) {
      rep.push(fCast);
    }
  });
  return rep;
