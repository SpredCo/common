  ...
  spredcastModel.find({ $or: [{ date: { $gt: new Date() }, state: 0}, { state: 1 }]}).populate('creator tags').exec(function (err, fCasts) {
    if (err) {
      ...
      fCasts.forEach(function (fCast) {
        if (filters.state === undefined || filters.state === fCasts.state || filters.state.indexOf(fCast.state) !== -1) {
          rep.push(fCast);
        }
      });
      cb(null, rep);
...
spredcastSchema.statics.removeCast = function (id, userId, cb) {
  if (!mongoose.Types.ObjectId.isValid(id) || !mongoose.Types.ObjectId.isValid(userId)) {
    ...
    spredcastSchema.findOne({_id: id, creator: userId}, function (err, fCast) {
      if (err) {
        cb(err, false);
      } else if (fCast == null) {
        cb(null, false);
