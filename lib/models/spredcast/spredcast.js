...
spredcastSchema.statics.findAvailableCast = function (filters, cb) {
  ...
  if (filters.tags === undefined) {
    spredcastModel.find({ $or: [{ date: { $gt: new Date() } }, stateCond] }).populate('creator tags').exec(cb);
  } else {
    spredcastModel.find({ $or: [{ date: { $gt: new Date() } }, stateCond], tags: tagCond }).populate('creator tags').exec(cb);
  }
};
...
spredcastSchema.statics.getNeedRemindCast = function (maxTime, cb) {
  spredcastModel.find({ reminded: 0, date: { $lt: maxTime } }, cb);
};
...
spredcastSchema.statics.reminded = function (ids, cb) {
  spredcastModel.update({ _id: { $in: ids } }, { reminded: 1 }, cb);
};
