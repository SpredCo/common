...
spredcastSchema.statics.createNew = function (creator, name, description, tags, date, isPublic, userCapacity, members, duration, url, cb) {
  if (date === 'now') {
    date = new Date();
  }
...
spredcastSchema.statics.findAvailableCast = function (cb) {
  spredcastModel.find({ $or: [{ date: { $gt: new Date() } }, { state: 1 }] }).populate('creator member').exec(cb);
};

spredcastSchema.statics.getByUrl = function (url, cb) {
  spredcastModel.findOne({ url: url }, cb);
};
