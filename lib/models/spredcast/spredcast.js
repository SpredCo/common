...
spredcastSchema.statics.findAvailableCast = function (filters, cb) {
  if (filters.tags === undefined) {
    spredcastModel.find({ $or: [{ date: { $gt: new Date() }, state: 0}, { state: 1 }], isPublic: true}).populate('creator tags').exec(function (err, fCasts) {
      if (err) {
        cb(err);
      } else {
        const cast = filterByState(filters, fCasts);
        cb(null, cast);
      }
    });
  } else {
    ...
    spredcastModel.find({ $or: [{ date: { $gt: new Date() }, state: 0}, { state: 1 }], tags: {$in: filters.tags}, isPublic: true}).populate('creator tags').exec(function (err, fCasts) {
      if (err) {
        cb(err);
      } else {
        const cast = filterByState(filters, fCasts);
...
function filterByState (filter, fCasts) {
  var rep = [];
  var states = [];
  if (filter.state !== undefined) {
    if (Array.isArray(filter.state)) {
        filter.state.forEach(function (elem) {
          states.push(parseInt(elem));
        });
    } else {
      filter.state = parseInt(filter.state)
    }
  }
  fCasts.forEach(function (fCast) {
    if (filter.state === undefined || filter.state === fCast.state || (Array.isArray(filter.state) && states.indexOf(fCast.state) !== -1)) {
      rep.push(fCast);
    }
  });
  return rep;
...
spredcastSchema.statics.removeCast = function (id, userId, cb) {
  if (!mongoose.Types.ObjectId.isValid(id) || !mongoose.Types.ObjectId.isValid(userId)) {
    ...
    spredcastModel.findOne({_id: id, creator: userId}, function (err, fCast) {
      if (err) {
        cb(err, false);
      } else if (fCast == null) {
        cb(null, false);
