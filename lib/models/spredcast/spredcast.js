...
spredcastSchema.statics.findAvailableCast = function (filters, cb) {
  ...
  if (filters.tags === undefined) {
    spredcastModel.find({ $or: [{ date: { $gt: new Date() } }, stateCond] }).populate('creator members tags').exec(cb);
  } else {
    spredcastModel.find({ $or: [{ date: { $gt: new Date() } }, stateCond], tags: tagCond }).populate('creator members tags').exec(cb);
  }
};
...
spredcastSchema.statics.getByTag = function (tags, cb) {
  spredcastModel.find({ $or: [{ date: { $gt: new Date() } }, { state: 1 }], tags: tags }).populate('creator members tags').exec(cb);
};
